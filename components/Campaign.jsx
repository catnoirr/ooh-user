  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (!user) {
        ...
        toast.error("Log in first and Scan the Code again");
  ...
  const fetchUser = async () => {
    try {
      ...
      if (data.coupons) {
        ...
        if (coupon) {
          toast.error("You have already scanned this code");
          router.push("/coupons");
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center">
      {showVideo ? (
        ...
        <>
       {
        spinWheel && <SpinWheel pricePerScan={pricePerScan} vendor={vendor} user={user} campaign={campaign} data={prizes} />
       }
      {
         quiz && campaign?.quizQuestions && <QuizComponent quizQuestions={campaign.quizQuestions} setSpinWheel={setSpinWheel} setQuiz={setQuiz} />
